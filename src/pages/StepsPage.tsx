...
import { CalendarDays, Check, Clock3, Edit3, Flame, Footprints, Gauge, Medal, Plus, Route, Settings2, Target, Trash2, TriangleAlert } from 'lucide-react'
...
export function StepsPage({ userId }: { userId: string }) {
  ...
  return <section className="steps-page">
    <div className="page-heading steps-hero">
      ...
      <div className="steps-hero-actions"><Button variant="secondary" onClick={openGoal}><Settings2 size={17} /> Meta diária</Button><Button onClick={openAdd}><Plus size={17} /> Registrar passos</Button></div>
    </div>
    ...
    <div className="steps-layout">
      ...
      <Card className="steps-history">
        <div className="steps-panel-heading"><div><small>REGISTROS RECENTES</small><h2>Histórico manual</h2></div><Button variant="secondary" onClick={openAdd}><Plus size={15} /> Novo</Button></div>
        <div className="steps-record-list">
          {recentRecords.map((record) => <article key={record.id}><span><Footprints size={17} /></span><div><strong>{formatNumber(record.steps)} passos</strong><small><CalendarDays size={12} /> {formatDate(record.occurredOn)} · {formatDistance(record.distanceKm)} · {formatDuration(record.durationMinutes)}</small></div><b>{formatNumber(Math.round(record.calories))} kcal</b><button onClick={() => openEdit(record)} aria-label="Editar registro"><Edit3 size={15} /></button><button onClick={() => void remove(record)} aria-label="Remover registro"><Trash2 size={15} /></button></article>)}
    ...
    <div className="steps-integration-note"><span><Route size={18} /></span><div><strong>Integrações automáticas em preparação</strong><p>A estrutura já distingue a origem dos dados para receber Apple Health e Health Connect futuramente. Nesta versão, todos os registros são manuais.</p></div></div>
