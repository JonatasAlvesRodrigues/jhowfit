...
const steps = [
  { title: 'InformaÃ§Ãµes bÃ¡sicas', short: 'VocÃª', icon: UserRound },
  { title: 'Seu objetivo', short: 'Objetivo', icon: Sparkles },
  { title: 'ExperiÃªncia', short: 'NÃ­vel', icon: Dumbbell },
  { title: 'Disponibilidade', short: 'Agenda', icon: HeartPulse },
  { title: 'Local e equipamentos', short: 'Estrutura', icon: MapPin },
  { title: 'AlimentaÃ§Ã£o', short: 'Dieta', icon: Salad },
  { title: 'SeguranÃ§a', short: 'SaÃºde', icon: ShieldAlert },
] as const
...
export function OnboardingPage({ userId, initialName, onComplete }: { userId: string; initialName: string; onComplete: () => void }) {
  ...
  async function finish() {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : 'NÃ£o foi possÃ­vel concluir sua configuraÃ§Ã£o.')
      setSaving(false)
    }
  }
    ...
    <div className="onboarding-shell">
      ...
      <div className="onboarding-layout">
        <aside className="onboarding-steps">
          <div className="onboarding-steps__intro"><small>SEU PERFIL</small><h2>Vamos personalizar o MOVELYA.</h2><p>Suas respostas ajudam a preparar uma experiÃªncia mais adequada Ã  sua rotina.</p></div>
          <nav aria-label="Etapas do questionÃ¡rio">
          ...
          <section className="onboarding-card">
            ...
            <footer className="onboarding-actions">
              ...
              {currentStep < steps.length - 1
                ...
                : <button className="onboarding-next" onClick={finish} disabled={saving}>{saving ? <><LoaderCircle className="spin" size={18} /> Salvando...</> : <>Concluir configuraÃ§Ã£o <Check size={17} /></>}</button>}
            </footer>
          </section>
        </main>
      </div>
...
const stepDescriptions = [
  ...
  'Qual resultado Ã© mais importante para vocÃª neste momento?',
  'Como vocÃª descreve sua experiÃªncia atual com exercÃ­cios?',
  'Encaixe os treinos na sua vida, e nÃ£o o contrÃ¡rio.',
  'Conte onde pretende treinar e o que terÃ¡ Ã  disposiÃ§Ã£o.',
  'Essas informaÃ§Ãµes ajudam a adaptar futuras sugestÃµes de refeiÃ§Ãµes.',
  'Sua seguranÃ§a vem primeiro. Responda com atenÃ§Ã£o.',
]
...
function BasicStep({ data, update }: { data: OnboardingData; update: Update }) {
  return <div className="onboarding-form-grid">
    ...
    <OnboardingField label="Sexo (opcional)"><select value={data.gender} onChange={(event) => update('gender', event.target.value)}><option value="">Prefiro nÃ£o informar</option><option value="female">Feminino</option><option value="male">Masculino</option><option value="non-binary">NÃ£o binÃ¡rio</option><option value="other">Outro</option></select></OnboardingField>
...
function AvailabilityStep({ data, update, toggle }: { data: OnboardingData; update: Update; toggle: (value: string) => void }) {
  return <div className="onboarding-form-grid">
    ...
    <OnboardingField label="DuraÃ§Ã£o mÃ©dia"><select value={data.averageDurationMinutes} onChange={(event) => update('averageDurationMinutes', event.target.value)}><option value="">Selecione</option><option value="20">20 minutos</option><option value="30">30 minutos</option><option value="45">45 minutos</option><option value="60">1 hora</option><option value="90">1h30</option><option value="120">2 horas</option></select></OnboardingField>
    <OnboardingField label="HorÃ¡rio preferido" className="is-wide"><div className="onboarding-segments">{[['morning','ManhÃ£'],['afternoon','Tarde'],['evening','Noite'],['flexible','FlexÃ­vel']].map(([id,label]) => <button key={id} className={data.preferredTime === id ? 'is-selected' : ''} onClick={() => update('preferredTime', id)}>{label}</button>)}</div></OnboardingField>
    <OnboardingField label="Dias disponÃ­veis" className="is-wide"><div className="weekday-picker">{weekDays.map(([id,label]) => <button key={id} className={data.availableDays.includes(id) ? 'is-selected' : ''} onClick={() => toggle(id)}>{label}</button>)}</div></OnboardingField>
  </div>
}
...
function LocationStep({ data, toggleList }: { data: OnboardingData; toggleList: (key: 'trainingLocations' | 'equipment', value: string) => void }) {
  return <div className="onboarding-stack">
    <OnboardingField label="Onde vocÃª pretende treinar?"><div className="selection-grid">{locationOptions.map(([id,label]) => <button key={id} className={data.trainingLocations.includes(id) ? 'is-selected' : ''} onClick={() => toggleList('trainingLocations', id)}><MapPin size={17} />{label}</button>)}</div></OnboardingField>
    <OnboardingField label="Equipamentos disponÃ­veis"><ChipPicker options={equipmentOptions} selected={data.equipment} onToggle={(value) => toggleList('equipment', value)} /></OnboardingField>
  </div>
}
...
function NutritionStep({ data, update, toggleList }: { data: OnboardingData; update: Update; toggleList: (key: 'dietaryPreferences' | 'dietaryRestrictions', value: string) => void }) {
  return <div className="onboarding-form-grid">
    <OnboardingField label="RefeiÃ§Ãµes por dia"><select value={data.mealsPerDay} onChange={(event) => update('mealsPerDay', event.target.value)}><option value="">Selecione</option>{[1,2,3,4,5,6].map((value) => <option value={value} key={value}>{value} refeiÃ§Ãµes</option>)}</select></OnboardingField>
    <OnboardingField label="OrÃ§amento mensal aproximado"><div className="unit-input is-prefix"><span>R$</span><input type="number" min="0" step="50" value={data.monthlyFoodBudget} onChange={(event) => update('monthlyFoodBudget', event.target.value)} placeholder="Opcional" /></div></OnboardingField>
    <OnboardingField label="PreferÃªncias alimentares" className="is-wide"><ChipPicker options={dietOptions} selected={data.dietaryPreferences} onToggle={(value) => toggleList('dietaryPreferences', value)} /></OnboardingField>
    <OnboardingField label="RestriÃ§Ãµes alimentares" className="is-wide"><ChipPicker options={restrictionOptions} selected={data.dietaryRestrictions} onToggle={(value) => toggleList('dietaryRestrictions', value)} /></OnboardingField>
    <OnboardingField label="Alimentos que nÃ£o consome"><textarea value={data.avoidedFoods} onChange={(event) => update('avoidedFoods', event.target.value)} placeholder="Ex.: peixe, brÃ³colis..." /></OnboardingField>
...
function SafetyStep({ data, update }: { data: OnboardingData; update: Update }) {
  const safetyItems: { key: 'injuries' | 'pain' | 'physicalLimitations' | 'healthConditions' | 'medication'; label: string; placeholder: string }[] = [
    { key: 'injuries', label: 'Possui alguma lesÃ£o?', placeholder: 'Descreva a lesÃ£o e a regiÃ£o afetada' },
    { key: 'pain', label: 'Sente dores recorrentes?', placeholder: 'Onde e com qual frequÃªncia?' },
    { key: 'physicalLimitations', label: 'Possui limitaÃ§Ãµes fÃ­sicas?', placeholder: 'Conte quais movimentos exigem cuidado' },
    { key: 'healthConditions', label: 'Possui condiÃ§Ãµes de saÃºde?', placeholder: 'Informe apenas o que for relevante' },
    { key: 'medication', label: 'Faz uso de medicamentos?', placeholder: 'Quais medicamentos?' },
  ]
  return <div className="onboarding-stack">
    <div className="medical-notice"><ShieldAlert size={21} /><p><strong>Sua seguranÃ§a Ã© prioridade.</strong><span>O MOVELYA nÃ£o substitui acompanhamento mÃ©dico, nutricional, fisioterapÃªutico ou de um profissional de educaÃ§Ã£o fÃ­sica. Consulte um profissional antes de iniciar mudanÃ§as relevantes.</span></p></div>
    ...
    <OnboardingField label="Gravidez"><div className="onboarding-segments pregnancy-options">{[['not-applicable','NÃ£o se aplica'],['no','NÃ£o'],['yes','Sim'],['prefer-not','Prefiro nÃ£o informar']].map(([id,label]) => <button key={id} className={data.pregnancyStatus === id ? 'is-selected' : ''} onClick={() => update('pregnancyStatus', id)}>{label}</button>)}</div></OnboardingField>
  </div>
}
...
function validateStep(step: number, data: OnboardingData) {
  if (step === 0) {
    ...
    if (new Date(data.birthDate) >= new Date()) return 'Informe uma data de nascimento vÃ¡lida.'
  ...
  if (step === 2 && !data.experienceLevel) return 'Selecione seu nÃ­vel de experiÃªncia.'
  if (step === 3) {
    if (!data.trainingDaysPerWeek) return 'Informe quantos dias pode treinar.'
    if (!data.averageDurationMinutes) return 'Selecione a duraÃ§Ã£o mÃ©dia disponÃ­vel.'
    if (!data.preferredTime) return 'Selecione seu horÃ¡rio preferido.'
  ...
  if (step === 4 && !data.equipment.length) return 'Selecione os equipamentos disponÃ­veis ou â€œNenhumâ€.'
  if (step === 5) {
    if (!data.mealsPerDay) return 'Informe a quantidade de refeiÃ§Ãµes.'
    if (!data.dietaryPreferences.length) return 'Selecione uma preferÃªncia alimentar.'
    if (!data.dietaryRestrictions.length) return 'Selecione uma restriÃ§Ã£o ou â€œNenhumaâ€.'
  }
  if (step === 6) {
    const details = [data.injuries, data.pain, data.physicalLimitations, data.healthConditions, data.medication]
    if (details.some((answer) => answer.value && answer.details.trim().length < 3)) return 'Descreva os itens de saÃºde marcados como â€œSimâ€.'
    if (!data.pregnancyStatus) return 'Selecione uma opÃ§Ã£o sobre gravidez.'
  }
  return ''
}
