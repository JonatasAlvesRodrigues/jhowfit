...
export const communityProfileRoute: VitaRoute = { id: 'perfil-social', path: '/perfil-social', label: 'Perfil da comunidade', eyebrow: 'EM MOVIMENTO', description: 'O ritmo e as conquistas compartilhadas na comunidade.', icon: UsersRound }
export function findRoute(pathname: string) { const normalized = (pathname === '/' ? '/inicio' : pathname.replace(/\/+$/, '').toLowerCase()).split('?')[0]; return [...vitaRoutes, communityProfileRoute, adminRoute, checkoutRoute, checkoutConfirmationRoute, ...authRoutes, onboardingRoute].find((route) => route.path === normalized) ?? null }
